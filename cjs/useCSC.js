...
exports.useCountryStateCity = exports.useCSC = exports.loadCitysByStateAndCountry = void 0;
...
const loadStatesByCountry = async (country) => {
    const { states } = await Promise.resolve(`${`country-state-city-nextjs/cjs/country/${(0, index_1.parseNameFolder)(country)}/states`}`).then(s => __importStar(require(s)));
    return states;
};
const loadCitysByStateAndCountry = async (country, state) => {
    const { citys } = await Promise.resolve(`${`country-state-city-nextjs/cjs/country/${(0, index_1.parseNameFolder)(country)}/${(0, index_1.parseNameFolder)(state)}/citys`}`).then(s => __importStar(require(s)));
    return citys;
};
